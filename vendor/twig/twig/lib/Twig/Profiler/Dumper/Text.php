...
class Twig_Profiler_Dumper_Text
{
    ...
    {
        return sprintf('%sРІвЂќвЂќ %s', $prefix, $profile->getTemplate());
    }

    protected function formatNonTemplate(Twig_Profiler_Profile $profile, $prefix)
    {
        return sprintf('%sРІвЂќвЂќ %s::%s(%s)', $prefix, $profile->getTemplate(), $profile->getType(), $profile->getName());
    }
